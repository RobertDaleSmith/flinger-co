<?php

?>

<!DOCTYPE HTML>
<!--       Developed by @RobertDaleSmith founder of @MoteLabs.           
           ____    ___  @FlingerCo / founders@flinger.co                                                       
          /\  _`\ /\_ \    __                                                             
          \ \ \L\_\//\ \  /\_\    ___      __      __   _ __      ___    ___   
           \ \  _\/ \ \ \ \/\ \ /' _ `\  /'_ `\  /'__`\/\`'__\   /'___\ / __`\ 
            \ \ \/   \_\ \_\ \ \/\ \/\ \/\ \L\ \/\  __/\ \ \/ __/\ \__//\ \L\ \
             \ \_\   /\____\\ \_\ \_\ \_\ \____ \ \____\\ \_\/\_\ \____\ \____/
              \/_/   \/____/ \/_/\/_/\/_/\/___L\ \/____/ \/_/\/_/\/____/\/___/ 
                                           /\____/                             
                                           \_/__/                  (C)(TM)2013
-->

<html lang="en">
    <head>
        <meta charset="utf-8" />
        <title>Flinger.co - Thanks You</title>

        <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
        <meta name="title" 		  content="Flinger.co - Thanks for Your Support!!" />
        <meta name="copyright" 	  content="MoteLabs.com - 2013" />
        <meta name="email" 		  content="contact@flinger.co" />
        <meta name="description" content="..." />
        <meta name="viewport" content="width=320, width=device-width, initial-scale=1, maximum-scale=1" /> <!-- This will force the device viewport to maintain the correct scale -->
        <meta name="apple-mobile-web-app-capable" content="yes" /> <!-- This will remove the Safari web browsers address bar & bookmarks bar giving your web page a native appearance -->

        <link href="/css/style_pages.css" media="all" rel="stylesheet" type="text/css" />
        
        <!-- Opera Speed Dial Favicon -->
        <link rel="icon" type="image/png" href="http://flinger.co/fav-icon-32.png" />
        <!-- Standard Favicon -->
        <link rel="icon" type="image/x-icon" href="http://flinger.co/favicon.ico" />
        <!-- For iPhone 4 Retina display: -->
        <link rel="apple-touch-icon-precomposed" sizes="114x114" href="http://flinger.co/apple-touch-icon-114x114-precomposed.png">
        <!-- For iPad: -->
        <link rel="apple-touch-icon-precomposed" sizes="72x72" href="http://flinger.co/apple-touch-icon-72x72-precomposed.png">
        <!-- For iPhone: -->
        <link rel="apple-touch-icon-precomposed" href="http://flinger.co/apple-touch-icon-57x57-precomposed.png">
        
        <meta property="fb:app_id" content="271277383008651" />
        <meta property="fb:admins" content="537257174" />  
        <meta property="og:title" content="Flinger.co - Thanks for Your Support!!" />
        <meta property="og:url" content="http://flinger.co/support/thank-you/" >
        <meta property="og:site_name" content="Flinger.co" />
        <meta property="og:description" content="..." />
        <meta property="og:type" content="website" />
        <meta property="og:image" content="http://flinger.co/icon_512.png" />

        <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">

        <script type="text/javascript" src="http://flinger.co/js/jquery.js"></script>

        <script>
          (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
          (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
          m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
          })(window,document,'script','//www.google-analytics.com/analytics.js','ga');

          ga('create', 'UA-39500794-5', 'flinger.co');
          ga('send', 'pageview');

        </script>

    </head>
    <body id="body_wrapper">

        <?php require '../../pages_header.php' ?>

        <script>
            $('#header_link_advert').css("color","#f4f4f4");
            $('#side_bar_page_title').html("ADVERTISE");
        </script>

        <div id="main_body_container">
            <div id="side_bar_container">
                <div id="side_bar_page_title">THANK YOU!</div>

                <div id="side_bar_dot_co_proud">
                    <script type="text/javascript" src="//s3.amazonaws.com/opportunityco/proud/proud.js"></script><script>COBadge.aff="btknp3";COBadge.show();</script>
                </div>
            </div>
            <div id="main_content_container">
                <div class="blog_post">
                    <br>
                    <div class="blog_post_title">You are now a Founding Member</div>
                    <div class="blog_post_body">    
                                            
                        As a founding member you will be among the first to be able to lock-in your own custom Flinger channel name with matching Flinger url. 
                        You are added to the list and we will contact you with updates as we move forward.
                        <br><br>
                        Be sure to follow us on your favorite social networks to stay connected to our regular updates, progress, and news.
                        <br><br>

                        <a href="https://angel.co/mote-labs-1?follow=1&type=Startup&id=88607"></a><script src="https://angel.co/javascripts/embed.js" type="text/javascript"></script>
                                    
                        <div class="fb-like" style="display: inline-block;position: relative;" data-href="http://facebook.com/flingerco" data-send="false" data-layout="button_count" data-width="450" data-show-faces="true" data-colorscheme="light"></div>&nbsp;&nbsp;&nbsp;&nbsp;
                        &nbsp;&nbsp;&nbsp;&nbsp;
                        <div class="g-follow" data-annotation="bubble" data-height="20" data-href="//plus.google.com/107129845099957054722" data-rel="publisher"></div>
                        &nbsp;&nbsp;
                        <div id="twitter_follow_container"><a href="https://twitter.com/FlingerCo" class="twitter-follow-button" data-show-count="true">Follow @FlingerCo</a></div>

                    </div>
                </div>                
            </div>
        </div>
        <?php require '../../pages_footer.php' ?>
        
    </body>
</html>
